...
/**
 * Props del componente CardMisEmisores
 * @property row - Fila de la tabla a renderizar que representa a los datos de cada registro
 */
type CardMisEmisoresProps = {
  readonly row: Row<TypeEmisor>;
};

/**
 * Componente CardMisEmisores - Tarjeta que muestra información de un registro en formato mobile.
 * @param {CardMisEmisoresProps} props - Propiedades del componente
 * @returns {JSX.Element} - Tarjeta de visualización de un registro
 */
export default function CardMisEmisores({
  row,
}: CardMisEmisoresProps): JSX.Element {
  /**
   * getCell - Obtiene la celda correspondiente al ID de columna especificada.
   * @param {string} cellId - ID de la columna a obtener
   * @returns {Cell<TypeEmisor, unknown> | undefined} - Celda que coincide con el ID, o undefined si no se encuentra
   */
  const getCell = (cellId: string): Cell<TypeEmisor, unknown> | undefined => {
  ...
  const cuentaEmisor = renderColumn("cuentaEmisor");
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          {cuentaEmisor && (
            <div className="flex items-center space-x-2 text-sm">
              ...
              <span className="font-medium">Cuenta del emisor:</span>
              <span>{cuentaEmisor}</span>
            </div>
          )}
          {idOrganizacion && (
            <div className="flex items-center space-x-2 text-sm">
