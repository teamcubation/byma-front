...
export function definirColumnas<T extends TypeGerente, K extends keyof T>(
  keys: Array<keyof T>,
  idKey: K,
  booleanKey: K,
...
export function definirColumnas<T extends TypeEmisor, K extends keyof T>(
  keys: Array<keyof T>,
  idKey: K,
  booleanKey: K,
  navigateFunction: NavigateFunction,
...
export function definirColumnas<T extends TypeEspecie, K extends keyof T>(
  keys: Array<keyof T>,
  idKey: K,
  booleanKey: K,
  navigateFunction: NavigateFunction,
...
export function definirColumnas<T, K extends keyof T>(
  keys: Array<keyof T>,
  idKey: K,
  booleanKey: K,
  navigateFunction: NavigateFunction,
      ...
      <div className="flex justify-center items-center gap-2">
        <TooltipProvider>
          <Tooltip>
            <TooltipTrigger>
              <Button
                ...
                onClick={() => {
                  console.log(row);
                  navigateFunction(`/editar-gerente/${row.original[idKey]}`, {
          ...
          <Tooltip>
            ...
            <TooltipContent>
              {row.original[booleanKey] ? (
                <p>Deshabilitar gerente</p>
              ) : (
                <p>Habilitar gerente</p>
              )}
            </TooltipContent>
          </Tooltip>
        </TooltipProvider>
...
export const MisGerentes = () => {
  ...
  const handleDelete = async (id: number) => {
    try {
      await axios.patch(
        `${
          import.meta.env.VITE_GERENTE_BASE_URL
        }/gerentes/toggle-habilitar/${id}`
      );
    ...
    <section className="p-6 flex flex-col gap-6">
      ...
      <Card className="p-4 bg-white rounded-xl shadow-md">
        <DataTable
          ...
          data={gerentes}
          Card={CardMisGerentes<TypeGerente>}
          booleanKey="habilitado"
        />
      </Card>
    </section>
  );
